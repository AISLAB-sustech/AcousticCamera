...
if (length(p) > 0)
  ...
  pIdxY = p+2;
  pIdxZ = p+3;
  s = scatter3(g.xs_gt(pIdxX), g.xs_gt(pIdxY), g.xs_gt(pIdxZ),'magenta','filled');
  s.SizeData = 10;
  % plot3(g.x(pIdxX), g.x(pIdxY), g.x(pIdxZ), 'LineStyle','none','Marker','s', 'MarkerSize', 4,'MarkerEdgeColor','c');
  % plot3(g.xs_gt(pIdxX), g.xs_gt(pIdxY), g.xs_gt(pIdxZ),'Color','magenta', 'LineStyle','none','Marker','o', 'MarkerSize', 4,'MarkerEdgeColor','magenta');
end
